...
export function OracleFavoriteStar({ oracleId, className }: OracleFavoriteStarProps) {
  // const utils removed (no trpc)
  
  const { data: favorites } = useApiQuery("/api/favorites/list");
  
  const isFavorited = favorites?.some(f => f.oracleId === oracleId) ?? false;
  
  const addMutation = useApiMutation("/api/favorites/add", {
    onSuccess: () => {
      // refetch handled by state;
  ...
  const removeMutation = useApiMutation("/api/favorites/remove", {
    onSuccess: () => {
      // refetch handled by state;
