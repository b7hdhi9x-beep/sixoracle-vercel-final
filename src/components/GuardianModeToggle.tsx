...
export function GuardianModeToggle({ compact = false, className = "" }: GuardianModeToggleProps) {
  ...
  const { data: settings, isLoading, refetch } = useApiQuery("/api/companion/settings") || { data: null, isLoading: false, refetch: () => {} } 
  };

  // @ts-ignore
  const updateMutation = useApiMutation("/api/companion/settings", {
    onSuccess: () => {
