...
export function IntimacyDisplay({ oracleId, compact = false, onLevelUp }: IntimacyDisplayProps) {
  ...
  const intimacyQuery = useApiQuery<any>(
    oracleId ? `/api/intimacy/oracle?oracleId=${oracleId}` : null,
    { enabled: !!oracleId }
  );
  const intimacy = intimacyQuery?.data ?? null;
  const isLoading = intimacyQuery?.isLoading ?? false;
  
  const rewardsQuery = useApiQuery<any[]>(
    oracleId ? `/api/rewards/list?oracleId=${oracleId}` : null,
    { enabled: !!oracleId }
  );
  const rewards = rewardsQuery?.data ?? [];
  
  // @ts-ignore
  const recordLoginMutation = useApiMutation("/api/intimacy/oracle");
...
export function IntimacyOverview() {
  ...
  const { data: allIntimacies, isLoading } = useApiQuery("/api/intimacy/all") || { data: [], isLoading: false };
