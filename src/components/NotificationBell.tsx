...
export function NotificationBell() {
  ...
  const { data: unreadCount } = useApiQuery("/api/notifications/unread-count", {
    refetchInterval: 30000, // Refetch every 30 seconds
  });

  const { data: notifications, refetch } = useApiQuery<any[]>(
    isOpen ? "/api/notifications/list" : null,
    { enabled: isOpen }
  );

  const markAsRead = useApiMutation("/api/notifications/read", {
    onSuccess: () => refetch(),
  });

  const markAllAsRead = useApiMutation("/api/notifications/read", {
    method: "POST",
    onSuccess: () => refetch(),
  });
