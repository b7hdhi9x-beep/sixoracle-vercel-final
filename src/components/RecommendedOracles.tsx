...
export function RecommendedOracles({ 
  ...
  const { data: recommendations, isLoading } = useApiQuery(`/api/chat/recommended?currentOracleId=${currentOracleId || ""}`, { enabled: true });

  // Mark referral as followed when user clicks on recommendation
  const markFollowedMutation = useApiMutation("/api/chat/recommended");
